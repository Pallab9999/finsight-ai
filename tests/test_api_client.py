...
def test_multi_company_evaluation_produces_different_scores():
    ...
    mec, _, _ = api_client.evaluate_application(DEMO, company_id="meccanica", loan_amount=500_000, force_mock=True)
    agro, _, _ = api_client.evaluate_application(DEMO, company_id="agrobio", loan_amount=400_000, force_mock=True)
...
def test_csv_upload_ingests_and_changes_score():
    ...
    payload, _, _ = api_client.evaluate_application(DEMO, company_id="meccanica", loan_amount=500_000, force_mock=True)
    assert payload["financial_score"] > 0


def test_auth_returns_user_on_valid_credentials():
    user, mode, msg = api_client.authenticate_api(
        "cfo@ecotex.it", "cfo2026", "sme_cfo", force_mock=True
    )
